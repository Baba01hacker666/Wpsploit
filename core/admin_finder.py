...
def check_admin_path(session, base_url, path):
    ...
    try:
        r = safe_get(session, url, timeout=get_default_timeout(), allow_redirects=True)
...
def find_admin_panels(session, base_url, threads):
    """Scans for admin panels using a list of common paths."""
    found_panels = []
    admin_paths = load_endpoints("endpoints/admin_paths.txt")

    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        future_to_path = {executor.submit(check_admin_path, session, base_url, path): path for path in admin_paths}
